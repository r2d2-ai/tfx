...
def _create_pipeline(pipeline_name: Text,
                     ...
                     model_framework: str) -> pipeline.Pipeline:
  ...
  Args:
    ...
    model_framework: the framework to use for defining the model,
      either 'keras' or 'flax'. Default is 'keras'.
  Returns:
  ...
  if enable_tuning:
    # TODO(necula): tuning is not yet supported for Flax.
    assert model_framework == 'keras'
    tuner = Tuner(
        module_file=module_file,
  ...
  trainer = Trainer(
      ...
      custom_config=dict(model_framework=model_framework),
      examples=transform.outputs['transformed_examples'],
      transform_graph=transform.outputs['transform_graph'],
      schema=schema_gen.outputs['schema'],
...
if __name__ == '__main__':
  ...
  LocalDagRunner().run(
      _create_pipeline(
          ...
          model_framework=flags.FLAGS.model_framework))
