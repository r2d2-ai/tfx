...
"""Supplement for penguin_utils.py with specifics for Flax models.
...
The code in this file is structures in two parts: the first part contains
typical Flax code to define and train a model. The second part is in the
class _SavedModelWrapper where we package the Flax trained model as a
tf.Model class.
...
_FEATURE_KEYS_XF = [
    'culmen_length_mm_xf', 'culmen_depth_mm_xf', 'flipper_length_mm_xf',
    'body_mass_g_xf'
]


def get_trained_model(train_data, eval_data, num_epochs: int,
                      steps_per_epoch: int, eval_steps_per_epoch: int,
                      tensorboard_log_dir: str):
  """Execute model training and evaluation loop.

  Args:
    train_data: an iterator of pairs, with the first element being a dictionary
      mapping the feature names from _FEATURE_KEYS_XF to np.ndarray values of
      shape f32[batch_size, 1] and the second element being an array of shape
      int64[batch_size, 1].
    eval_data: an iterator like `train_data` but with possibly a different batch
      size.
  ...
  def predict(params, inputs):
    return FlaxPenguinMLP().apply({'params': params}, inputs)

  trained_params = optimizer.target
  return _SavedModelWrapper.from_flax_prediction(predict, trained_params)


class FlaxPenguinMLP(nn.Module):
  """A simple MLP model."""

  @nn.compact
  def __call__(self, x_dict: Dict[str, Array]):
...
def _train_epoch(optimizer, train_data, steps_per_epoch):
  """Train for a single epoch."""
  batch_metrics = []
  for _ in range(steps_per_epoch):
    inputs, labels = next(train_data)
    optimizer, metrics = _train_step(optimizer, inputs, labels)
...
def _eval_epoch(params, eval_data, steps_per_epoch):
  """Validate for a single epoch."""
  batch_metrics = []
  for _ in range(steps_per_epoch):
    inputs, labels = next(eval_data)
    metrics = _eval_step(params, inputs, labels)
    batch_metrics.append(metrics)
...
@jax.jit
def _train_step(optimizer, inputs, labels):
  """Train for a single step, given a batch of inputs and labels."""

  def loss_fn(params):
    logits = FlaxPenguinMLP().apply({'params': params}, inputs)
...
@jax.jit
def _eval_step(params, inputs, labels):
  logits = FlaxPenguinMLP().apply({'params': params}, inputs)
  return _compute_metrics(logits, labels)


def _categorical_cross_entropy_loss(logits, labels):
  # logits: f32[B, 3]  (assumes that the logits use log_softmax activations)
  # labels: int64[B, 1]
...
class _SavedModelWrapper(tf.train.Checkpoint):
  ...
  def save(self, model_dir, *, save_format='tf', signatures):
    """Saves the model.
    ...
    Args:
      ...
      save_format: Exists only for compatibility with how penguin_utils.run_fn
        invokes the function. Should have the value 'tf'.
